  ...
  $result = preg_replace_callback('#'.unless_in_url_tag($pattern).'#i',
    function ($matches) use ($embed, $host, $pattern, $google_key) {
      ...
      if ($embed) {
          ...
          $new_body = '[iframe id="youtube-video" type="text/html" width="480" height="320" src="//www.youtube-nocookie.com/embed/' . $id . '?enablejsapi=1'.$params.'&wmode=transparent&origin=http://' . $host . '" frameborder="0" allowfullscreen]';
          if (isset($title)) {
...
function like($user_id, $msg_id, $val=1, $reaction=null) {
  ...
  //update the reaction event to support notifications
  //1. whose message?
  ...
  $query = 'INSERT INTO confa_events(item_owner_id,  item_id, event_owner_id, event_type) values(' .
    $msgAuthor . ', ' . $msg_id . ', ' . $user_id . ', 0)';
  $result = mysql_query($query);
