...
require_once('get_params_inc.php');
    ...
    if (!is_null($auth_cookie) && !is_null($user)) {
        $query = 'SELECT u.id, u.status, u.ban, u.prop_tz, u.moder, u.prop_bold, u.ban_ends, u.new_pm, u.username, s.user_id, s.hash, s.updated, u.last_pm_check_time from confa_users u, confa_sessions s where u.id = s.user_id and s.hash =\'' . $auth_cookie . '\' and u.username = \'' . mysql_real_escape_string($user) . '\'';
